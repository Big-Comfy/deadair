...
1. **One-shot scan, human eyes.** Run `deadair scan` and read the report with the team before
   ...
   deployment usually carries a real backlog (a stock Elastic install starts with 45 dead
   prebuilt rules). Triage severity-first like any detection-gap list — ticket what needs a
